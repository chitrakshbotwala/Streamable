...
export const useAniList = (session, stats) => {
  ...
  `;
    ...
    if (media) {
      ...
      if (!checkList?.includes("Watched using Streamable")) {
        checkList.push("Watched using Streamable");
      ...
      let lists = media?.mediaListEntry?.customLists
        ? Object.entries(media?.mediaListEntry?.customLists)
            ...
            .map(([key, value]) => key) || []
        : [];
      if (!lists?.includes("Watched using Streamable")) {
        lists.push("Watched using Streamable");
      }
      if (lists.length > 0) {
